...
import Pressure.Language.Types (BinaryOp (..), Mutability (Constant, Mutable), UnaryOp (..))
...
testParseProgram = do
  ...
  expect "constant declaration" (case singleDecl ast2 of Just (ParsedValueDecl Constant _ (Just typ) (ParsedExpr _ (ParsedIntLit _))) -> isIntSyntax typ; _ -> False) ast2

  ast4 <- parse "parse addition expression" "sum: i32 = 1 + 2 + 3;"
  expect "addition expression" (case singleDecl ast4 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedBinaryExpr AddOp (ParsedExpr _ (ParsedBinaryExpr AddOp (ParsedExpr _ (ParsedIntLit 1)) (ParsedExpr _ (ParsedIntLit 2)))) (ParsedExpr _ (ParsedIntLit 3))))) -> isIntSyntax typ; _ -> False) ast4

  ast5 <- parse "parse multiplication precedence" "value: i32 = 1 + 2 * 3;"
  expect "multiplication precedence" (case singleDecl ast5 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedBinaryExpr AddOp (ParsedExpr _ (ParsedIntLit 1)) (ParsedExpr _ (ParsedBinaryExpr MulOp (ParsedExpr _ (ParsedIntLit 2)) (ParsedExpr _ (ParsedIntLit 3))))))) -> isIntSyntax typ; _ -> False) ast5

  ast6 <- parse "parse division expression" "value: i32 = 8 / 4 / 2;"
  expect "division expression" (case singleDecl ast6 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedBinaryExpr DivOp (ParsedExpr _ (ParsedBinaryExpr DivOp (ParsedExpr _ (ParsedIntLit 8)) (ParsedExpr _ (ParsedIntLit 4)))) (ParsedExpr _ (ParsedIntLit 2))))) -> isIntSyntax typ; _ -> False) ast6

  ast7 <- parse "parse parenthesized expression" "value: i32 = (1 + 2) * 3;"
  expect "parenthesized expression" (case singleDecl ast7 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedBinaryExpr MulOp (ParsedExpr _ (ParsedBinaryExpr AddOp (ParsedExpr _ (ParsedIntLit 1)) (ParsedExpr _ (ParsedIntLit 2)))) (ParsedExpr _ (ParsedIntLit 3))))) -> isIntSyntax typ; _ -> False) ast7
  ...
  expect "subtraction precedence" (case singleDecl ast10 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedBinaryExpr SubOp (ParsedExpr _ (ParsedIntLit 1)) (ParsedExpr _ (ParsedBinaryExpr MulOp (ParsedExpr _ (ParsedIntLit 2)) (ParsedExpr _ (ParsedIntLit 3))))))) -> isIntSyntax typ; _ -> False) ast10
  ...
  expect "if expression" (case singleDecl ast13 of Just (ParsedValueDecl Mutable _ (Just typ) (ParsedExpr _ (ParsedIfExpr _ _ (Just _)))) -> isIntSyntax typ; _ -> False) ast13
  ...
  expect "function expression" (case singleDecl ast19 of Just (ParsedValueDecl Constant _ Nothing (ParsedExpr _ (ParsedFnExpr [Param _ p1, Param _ p2] ret _))) -> isIntSyntax p1 && isIntSyntax p2 && isIntSyntax ret; _ -> False) ast19
