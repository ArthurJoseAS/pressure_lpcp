...
testReplRecursiveFunction = do
  ...
  case checkReplWithEnv [] decl of
    ...
    Right (typedDecl, typeEnv) -> do
      result1 <- runExceptT (runStateT (evalRepl typedDecl) emptyEnv)
      case result1 of
        ...
        Right (_, env) ->
          case checkReplWithEnv typeEnv expr of
            ...
            Right (Repl [typedExpr], _) -> do
              result2 <- runExceptT (runStateT (evalReplInput typedExpr) env)
              case result2 of
